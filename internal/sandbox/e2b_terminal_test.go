...
func TestE2BTerminalInteractiveProtocol(t *testing.T) {
	...
	defer cancel()
	term, err := streamClient.ExecStream(ctx, handle, RemoteStreamExecRequest{Command: []string{"bash", "-l"}, Env: []string{"PROMPT_COMMAND=echo audit"}, WorkDir: "/workspace/output", Cols: 80, Rows: 24})
	require.NoError(t, err)
	defer term.Close()
	...
	require.Contains(t, string(start), "PROMPT_COMMAND")
	require.Contains(t, string(start), "echo audit")
